...
function PartnerShowcaseContent() {
  ...
  return (
    <div id="home" className="min-h-screen bg-surface-base text-text-primary selection:bg-brand-soft">
      <div className="relative z-10 flex min-h-screen flex-col">
        <main className="flex-1 pb-16 pt-2 lg:pt-4">
              ...
              <div className="flex w-full flex-col justify-between gap-4 px-4 py-4 md:flex-row md:px-6">
                ...
                <div className="flex items-center self-start rounded-lg bg-brand-soft p-1 md:self-auto">
                  <button
                    ...
                    className={`inline-flex h-9 w-9 items-center justify-center rounded-lg transition-all ${view === 'grid' ? 'bg-surface-card text-text-primary shadow-sm' : 'text-text-muted hover:text-text-primary'}`}
                  >
                    <iconify-icon icon="solar:widget-3-linear" width={20} />
                  </button>
                  <button
                    onClick={() => setView('list')}
                    className={`inline-flex h-9 w-9 items-center justify-center rounded-lg transition-all ${view === 'list' ? 'bg-surface-card text-text-primary shadow-sm' : 'text-text-muted hover:text-text-primary'}`}
                  >
              ...
              >
                {filteredPartners.map((p) => (
                  <div
                    key={p.id}
                    onClick={() => openModal(p)}
                    className={`group cursor-pointer border border-border bg-surface-card transition-all hover:-translate-y-0.5 hover:border-brand-ring hover:shadow-xl hover:shadow-brand-soft/60 animate-fade-in-up ${view === 'grid'
                    ...
                    {view === 'grid' ? (
                      <>
                        ...
                        <div className="p-6 grow flex flex-col">
                          ...
                          <p className="mb-4 grow line-clamp-2 text-sm text-text-secondary">{p.description}</p>
                ...
                <div className="flex-1 px-5 py-6 sm:px-8 sm:py-8">
                  <div className="mx-auto max-w-3xl space-y-8">
                    <section className="rounded-2xl border border-border bg-surface-section p-5">
                      ...
                      <p className="text-sm leading-relaxed text-text-secondary sm:text-base">{selectedPartner.description}</p>
                    </section>
                      ...
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div className="rounded-xl border border-border bg-surface-section p-4">
                          <div className="mb-3 flex items-center gap-3">
                            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-surface-card text-state-danger ring-1 ring-border">
                        ...
                        <div className="rounded-xl border border-brand bg-brand-soft/60 p-4">
                          <div className="mb-3 flex items-center gap-3">
                            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-surface-card text-brand ring-1 ring-brand-ring">
                    ...
                    <div className="pb-2">
                      <button className="flex w-full items-center justify-center gap-2 rounded-xl bg-button-primary px-4 py-3 text-sm font-medium text-button-text shadow-sm transition-colors hover:bg-button-primary-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand">
    ...
    <Suspense
      fallback={
        <div className="min-h-screen bg-surface-base text-text-primary selection:bg-brand-soft">
          <div className="relative z-10 mx-auto flex min-h-screen max-w-[1600px] items-center justify-center px-6">
